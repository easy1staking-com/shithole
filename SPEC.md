...
- **Swap-history lineage tracking.** Every listing UTxO ever observed at a curated address is recorded in an append-only `listing_events` table — genesis (initial pay-to-script with `update_ref == None`), each swap (with `update_ref_hash = compute_output_tag(consumed_outref)` resolved back to the consumed outref via lineage lookup), and the terminal action (cancel or recover). Slot → timestamp joined from Yaci Store's `block` table. Endpoint `GET /api/listings/{initial_outref}/history` returns the timeline. The `Listing` DTO's `update_ref` field surfaces the *previous* outref (resolved BE-side) so the FE can render "swapped on slot X" without doing hash reversal client-side. Full schema in [docs/BACKEND.md](./docs/BACKEND.md#swap-history-lineage-tracking).
- **Curation registry + config registration.** Curated collections are populated via `POST /api/configs`. The FE submits `{config_nft_policy, slug, display_name, theme, display_order, signature}` after the admin's config-deployment tx is confirmed on-chain. The BE trustlessly resolves the config UTxO from Blockfrost (script address = `Address(ScriptCredential(config_nft_policy))`), enforces asset-shape strictness (exactly one asset under the policy, 28-byte asset name, quantity 1), decodes the inline datum, enforces invariants (M within `[1, MAX_M]`, `protocol_fee` within `[0, MAX_FEE]`, `lister_fee` within `[MIN_LISTER_FEE, MAX_FEE]`, treasury credential variants supported), and verifies a CIP-8 (COSE_Sign1) signature from the on-chain `admin_pkh` over a canonical newline-delimited payload binding `(policy, slug, display_name, display_order, theme)`. On success both `configs` and `curated_collections` are written in a single short transaction; concurrent submits race the unique indexes and surface as 409. The `collection_policy_id` is derived from the config NFT's asset name; no separate field needed. CIP-171 auto-discovery is deferred for v1 — full flow + canonical payload spec + deferred-discovery roadmap in [docs/BACKEND.md](./docs/BACKEND.md#config-registration).
...
1. The protocol operator deploys a config UTxO via the one-shot mint path (§5.1) from the FE admin form. The deploy tx mints the config NFT and lands the config UTxO with an inline `ConfigDatum`.
2. On confirmation the FE posts to `POST /api/configs` with `{config_nft_policy, slug, display_name, theme, display_order, signature}`. The `signature` block is a CIP-8 (COSE_Sign1) signature from the on-chain `admin_pkh` over a canonical newline-delimited payload binding all the metadata fields — produced by the admin's wallet via `wallet.signData(adminAddress, hex(canonicalPayload))`.
3. The BE trustlessly verifies: resolves the config UTxO on-chain via Blockfrost, enforces asset-shape strictness (exactly one asset under the policy, 28-byte asset name, quantity 1), decodes the inline datum, enforces numeric invariants + upper bounds, then verifies the CIP-8 signature against `datum.admin_pkh` (recovers the Ed25519 pubkey from COSE_Key, hashes via blake2b-224, compares; then verifies the rebuilt SigStructure with the recovered pubkey). All in a single `POST /api/configs` request.
4. On success the BE writes both `configs` (live datum state) and `curated_collections` (slug + theme + display_order) in one short transaction. The collection becomes visible at `GET /api/curated` immediately.

The off-chain semantic check (that the policy id corresponds to a real on-chain collection with non-trivial activity, rather than malicious garbage) is the responsibility of the admin doing the deployment — the BE only certifies that the on-chain config UTxO is well-formed and admin-signed for this slug.

CIP-171 auto-discovery is **deferred for v1**: the FE-driven path is simpler to ship and keeps curation gated by an explicit admin signature. The deferred path remains documented in `docs/BACKEND.md` under "config registration" → "future: CIP-171 auto-discovery".

No "retire" path exists on-chain. To stop surfacing a collection, the operator removes the row from `curated_collections`. The on-chain config and any outstanding listings remain spendable in perpetuity (cancel by lister always works).
